...
-- Analytical View for User Metrics Dashboard
CREATE VIEW IF NOT EXISTS v_user_dashboard_metrics AS
SELECT 
    u.id AS user_id,
    u.name AS user_name,
    u.accessibility_mode,
    COALESCE(SUM(s.duration_seconds) / 3600.0, 0) AS total_listening_hours,
    COALESCE(AVG(c.confidence_score), 0) AS average_confidence_score,
    COALESCE(AVG(q.vocal_confidence_index), 0) AS voice_confidence_index,
...
GROUP BY u.id, u.name, u.accessibility_mode;
